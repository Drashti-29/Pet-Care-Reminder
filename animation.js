...
var Dog = {
    ...
    scale: 3,
        ...
        var scaledWidth = 82 * this.scale;
    var scaledHeight = 61 * this.scale;
			  this.imgDog.src = 'pug-running_transparent.png';
			  context.drawImage(this.imgDog, currFrameX, this.frameHeight, 82, 61, this.xDog, this.yDog, scaledWidth, scaledHeight);
			} 
}
 ...
 */
function particle() {
  this.pointX = randomIntFromInterval(canvas.width / 4, 5 * canvas.width / 4);
  this.pointY = randomIntFromInterval(canvas.height / 4, 400);
  this.horDrctn = this.pointX < canvas.height / 2 ? "left" : "right";
  this.alpha = 1;
  this.radius = 4; // Increased radius to zoom in on dots

  this.drawParticle = function(pointX, pointY, context) {
    context.beginPath();
    context.fillStyle = "#D95D50";
    context.arc(pointX, pointY, this.radius, 0, Math.PI * 2, false);
    context.fill();
    context.closePath();
  }
}
...
setInterval(function() {
  for (var j = 0; j < particleArray.length; j++) {
    if (particleArray[j].horDrctn === "left" && particleArray[j].pointX >= (5 * canvas.width) / 4) {
    ...
    if (particleArray[j].pointY >= 410) {
      particleArray[j].alpha = 0;
      particleArray[j].pointX = randomIntFromInterval(canvas.width / 4, 5 * canvas.width / 4);
      particleArray[j].pointY = randomIntFromInterval(canvas.height / 4, 400);
